...
export class LayoutService {
  ...
  // Signals para la configuración y estado
  ...
  constructor(private http: HttpClient) {
    // Lectura de la configuración previa desde localStorage (si existe)
   ...
   *  - Siempre se guarda la configuración completa (incluyendo darkTheme) en localStorage.
   *  - Si estamos en desarrollo (environment.production es false y environment.mockLogin es true),
   *    se usa únicamente localStorage.
   *  - En caso contrario, se realiza una llamada HTTP PUT a la API para actualizar la configuración en el backend,
   *    enviando solo el resto de la configuración (excluyendo darkTheme).
   */
  onConfigUpdate() {
    ...
    if (!environment.production && environment.mockLogin) {
      // En desarrollo, usando mock, se utiliza solo localStorage.
      console.log('Mock update layout config (guardado en localStorage):', currentConfig);
      this.configUpdate.next(currentConfig);
    } else {
      // En producción o cuando mockLogin es false, se actualiza en el backend.
      // Se envía el objeto sin darkTheme, ya que este se gestiona en localStorage.
